...
def reflect_image_vertical(matrix):
    if not matrix: return []
    return [row[:] for row in reversed(matrix)]

# reflect_image_horizontal
def reflect_image_horizontal(matrix):
    if not matrix: return []
...
# 3. 뒤집기
reflected_v_matrix = reflect_image_vertical(original_matrix)
print_matrix(reflected_v_matrix, "Vertically Reflected Matrix")
visualize_matrix(reflected_v_matrix, "Vertically Reflected")

reflected_h_matrix = reflect_image_horizontal(original_matrix)
print_matrix(reflected_h_matrix, "Horizontally Reflected Matrix")
visualize_matrix(reflected_h_matrix, "Horizontally Reflected")
